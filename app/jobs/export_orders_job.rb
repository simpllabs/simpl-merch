  ...
  def perform
    begin
      csv_string = CSV.generate do |csv|
        csv << ["Order ID", "Tracking Number", "Shop Domain", "Status", "Product", "Quantity", "Shipping Name", "Shipping Address1", "Shipping Address2", "Shipping Company", "Shipping City", "Shipping ZIP", "Shipping Province/State", "Shipping Country"]
        ...
        Order.where(created_at: from_date..to_date).each do |order|
          if order.shop_domain == @domain
            csv << [order.id, order.tracking_number, order.shop_domain, order.fulfillment_status, order.product_name, order.quantity, order.name, order.address1, order.address2, order.company, order.city, order.zip, order.province, order.country]
          end
        end
